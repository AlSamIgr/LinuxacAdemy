...
data "aws_ssm_parameter" "linuxAmiFrankfurt" {
...
resource "aws_instance" "jenkins-worker-frankfurt" {
  provider                    = aws.region-worker
  count                       = var.workers-count
  ami                         = data.aws_ssm_parameter.linuxAmiFrankfurt.value
  instance_type               = var.instance-type
  key_name                    = aws_key_pair.worker-key.key_name
  associate_public_ip_address = true
  vpc_security_group_ids      = [aws_security_group.jenkins-sg-frankfurt.id]
  subnet_id                   = aws_subnet.subnet_1_frankfurt.id
