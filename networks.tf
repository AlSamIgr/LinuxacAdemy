...
resource "aws_subnet" "subnet_1_frankfurt" {
  provider   = aws.region-worker
  vpc_id     = aws_vpc.vpc_master_frankfurt.id
...
resource "aws_vpc_peering_connection" "euwest1-eucentral1" {
  provider    = aws.region-master
  peer_vpc_id = aws_vpc.vpc_master_frankfurt.id
  vpc_id      = aws_vpc.vpc_master.id
  peer_region = var.region-worker

}

#Accept VPC peering request in eu-central-1 from eu-west-1
resource "aws_vpc_peering_connection_accepter" "accept_peering" {
  provider                  = aws.region-worker
  vpc_peering_connection_id = aws_vpc_peering_connection.euwest1-eucentral1.id
  auto_accept               = true
}
...
resource "aws_route_table" "internet_route_frankfurt" {
  provider = aws.region-worker
  vpc_id   = aws_vpc.vpc_master_frankfurt.id
  route {
    cidr_block = "0.0.0.0/0"
...
resource "aws_main_route_table_association" "set-worker-default-rt-assoc" {
  provider       = aws.region-worker
  vpc_id         = aws_vpc.vpc_master_frankfurt.id
  route_table_id = aws_route_table.internet_route_frankfurt.id
}
